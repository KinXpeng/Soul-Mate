...
// worker/instant-push/node_modules/@rei-standard/amsg-shared/dist/index.mjs
...
function buildReasoningPush(args) {
	...
	if (typeof args.reasoningContent !== 'string' || !args.reasoningContent) {
		throw new Error(
			"[amsg-shared] ReasoningPush: 'reasoningContent' must be a non-empty string"
		)
	}
...
function buildToolRequestPush(args) {
	...
	if (!Array.isArray(args.toolCalls) || args.toolCalls.length === 0) {
		throw new Error(
			"[amsg-shared] ToolRequestPush: 'toolCalls' must be a non-empty array"
		)
	}
...
function validateNotificationArg(kind, value) {
	...
	if (value === null || typeof value !== 'object' || Array.isArray(value)) {
		throw new Error(
			`[amsg-shared] ${kind}: 'notification' must be a plain object`
		)
	}
	const n =
	...
	if (
		n.show !== void 0 &&
		!['auto', 'always', 'when-hidden', false].includes(n.show)
	) {
		throw new Error(
			`[amsg-shared] ${kind}: 'notification.show' must be "auto", "always", "when-hidden", or false`
		)
	}
	for (const f of ['title', 'body', 'icon', 'badge', 'tag']) {
		if (n[f] !== void 0 && typeof n[f] !== 'string') {
			throw new Error(
				`[amsg-shared] ${kind}: 'notification.${f}' must be a string when present`
			)
		}
	}
	for (const f of ['renotify', 'requireInteraction']) {
		if (n[f] !== void 0 && typeof n[f] !== 'boolean') {
			throw new Error(
				`[amsg-shared] ${kind}: 'notification.${f}' must be a boolean when present`
			)
		}
	}
	if (
		...
		throw new Error(
			`[amsg-shared] ${kind}: 'notification.data' must be a plain object when present`
		)
	}
}
...
// worker/instant-push/node_modules/@rei-standard/amsg-instant/dist/adapters/cloudflare.mjs
...
var VALID_MESSAGE_ROLES = /* @__PURE__ */ new Set([
	'system',
	'user',
	'assistant',
	'tool'
...
function validateInstantPayload(payload, opts) {
	...
	if (payload.firstSendTime !== void 0) {
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'amsg-instant \u4E0D\u63A5\u53D7 firstSendTime\uFF1B\u8BE5\u5B57\u6BB5\u5C5E\u4E8E amsg-server \u7684\u5B9A\u65F6\u6D88\u606F',
	...
	if (payload.recurrenceType !== void 0 && payload.recurrenceType !== 'none') {
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'amsg-instant \u4E0D\u652F\u6301 recurrenceType\uFF08\u5FC5\u987B\u7701\u7565\u6216\u4E3A "none"\uFF09',
	...
	if (payload.messageType !== void 0 && payload.messageType !== 'instant') {
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'amsg-instant \u4EC5\u652F\u6301 messageType: "instant"\uFF08\u6216\u7701\u7565\uFF09',
	...
	if (hookPath && hasCompletePrompt) {
		return {
			...
			errorMessage:
				'completePrompt is not supported when onLLMOutput is configured; pass a `messages` array directly',
			details: {
				invalidFields: ['completePrompt'],
				hint: 'pass messages array directly'
			}
		}
	}
	if (hasCompletePrompt && hasMessages) {
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'exactly one of `completePrompt` or `messages` must be provided\uFF08\u4E24\u8005\u4E0D\u80FD\u540C\u65F6\u51FA\u73B0\uFF09',
	...
	if (!hasCompletePrompt && !hasMessages) {
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'exactly one of `completePrompt` or `messages` must be provided',
			details: { missingFields: ['completePrompt', 'messages'] }
		}
	}
	if (hasCompletePrompt) {
		if (
			typeof payload.completePrompt !== 'string' ||
			!payload.completePrompt.trim()
		) {
			return {
				valid: false,
				errorCode: 'INVALID_PAYLOAD_FORMAT',
				errorMessage:
					'completePrompt \u5FC5\u987B\u662F\u975E\u7A7A\u5B57\u7B26\u4E32',
	...
	if (
		...
		return {
			...
			errorCode: 'INVALID_PAYLOAD_FORMAT',
			errorMessage:
				'pushSubscription \u5FC5\u987B\u662F\u5408\u6CD5 Web Push \u8BA2\u9605\u5BF9\u8C61',
	...
	if (avatarErr) {
		console.warn(
			'[amsg-instant] avatarUrl \u4E0D\u5408\u6CD5\uFF0C\u5DF2\u7F6E\u7A7A\uFF1A',
			avatarErr
		)
		payload.avatarUrl = null
	}
	if (
		payload.messageSubtype !== void 0 &&
	...
	const removedField = [
		...
		'errorSplitPattern'
	].find(field => payload[field] !== void 0)
	if (removedField) {
		return {
			valid: false,
			errorCode: 'INVALID_PAYLOAD_FORMAT',
...
function validateContinuePayload(payload, opts) {
	...
	if (payload.completePrompt !== void 0) {
		return {
			...
			errorCode: 'COMPLETE_PROMPT_NOT_SUPPORTED_ON_HOOK_PATH',
			errorMessage:
				'completePrompt is not accepted on /continue; pass a `messages` array directly',
			details: { invalidFields: ['completePrompt'] }
		}
	}
	...
	if (avatarErr) {
		console.warn(
			'[amsg-instant] /continue avatarUrl \u4E0D\u5408\u6CD5\uFF0C\u5DF2\u7F6E\u7A7A\uFF1A',
			avatarErr
		)
		payload.avatarUrl = null
	}
...
function validateHookPathSharedFields(payload, opts) {
	...
	if (payload.iteration !== void 0) {
		const maxLoop =
			opts &&
			Number.isInteger(opts.maxLoopIterations) &&
...
async function sendWebPush({
	...
	if (
		...
		throw new Error(
			'sendWebPush: subscription.keys.p256dh and .auth are required'
		)
	}
...
async function buildVapidJwt({ audience, subject, publicKey, privateKey }) {
	...
	if (pubBytes.length !== 65 || pubBytes[0] !== 4) {
		throw new Error(
			'VAPID publicKey must be a 65-byte uncompressed P-256 point (base64url).'
		)
	}
...
function normalizeVapidSubject(email) {
	...
	if (!trimmed) return ''
	return /^mailto:/i.test(trimmed) || /^https?:/i.test(trimmed)
		? trimmed
		: `mailto:${trimmed}`
}
...
var PayloadTooLargeError = class extends Error {
	...
	constructor(byteLength, maxInlineBytes, opts) {
		super(
			`pushPayload UTF-8 byte length ${byteLength} exceeds maxInlineBytes ${maxInlineBytes}`,
			opts
		)
...
function buildMultipartPushPayloads(payload, options = {}) {
	...
	const ttlMs = resolvePositiveInteger(
		options.ttlMs,
	...
	const id =
		typeof options.id === 'string' && options.id.trim()
			...
			: `mp_${randomUUID()}`
	let serialized =
		typeof options.serializedPayload === 'string'
	...
	if (serialized === void 0) {
		try {
			...
			throw new TypeError(
				`buildMultipartPushPayloads: payload is not JSON-serializable: ${error?.message ?? error}`
			)
		}
	}
	if (typeof serialized !== 'string') {
		throw new TypeError(
			'buildMultipartPushPayloads: payload serialized to a non-string'
		)
	}
	const bytes = utf8(serialized)
	const total = Math.max(1, Math.ceil(bytes.byteLength / maxChunkBytes))
	const createdAt = Date.now()
	const originalMessageKind =
	...
	for (let i = 0; i < total; i++) {
		...
		parts.push({
			...
			multipart: {
				...
				originalMessageKind:
					typeof originalMessageKind === 'string' ? originalMessageKind : null,
				createdAt,
				ttlMs
			},
...
function resolvePositiveInteger(value, fallback, fieldName) {
	...
	if (!Number.isInteger(value) || value <= 0) {
		throw new TypeError(
			`buildMultipartPushPayloads: ${fieldName} must be a positive integer`
		)
	}
	return value
}
...
async function sendPushesSequentially(
	pushPayloads,
	payload,
	ctx,
	sessionId,
	...
	for (let i = 0; i < total; i++) {
		...
		try {
			...
			if (
				err &&
				(err.code === 'HOOK_THREW' || err.code === 'PAYLOAD_TOO_LARGE')
			) {
				throw err
			}
...
function buildAiRequestBody(payload) {
	...
	if (!Number.isInteger(payload.maxTokens) || payload.maxTokens <= 0) {
		throw new Error(
			'Invalid maxTokens: must be a positive integer when provided.'
		)
	}
...
async function callLlmRaw(payload, fetchImpl, requireContent) {
	...
	if (!res.ok) {
		...
		throw new Error(
			`AI API error: ${res.status} ${statusText}. Request URL: ${url}`
		)
	}
	...
	if (requireContent) {
		if (typeof rawContent !== 'string' || !rawContent.trim()) {
			throw new Error(
				'AI API error: response missing choices[0].message.content'
			)
		}
	}
	return {
...
async function runAgenticLoop(payload, ctx) {
	...
	const maxLoopIterations =
		Number.isInteger(ctx.maxLoopIterations) && ctx.maxLoopIterations > 0
	...
	while (iteration < maxLoopIterations) {
		...
		try {
			...
			try {
				...
				onEvent({
					type: 'diagnostic_push_failed',
					code: 'HOOK_THREW',
					sessionId,
					cause: pushErr
				})
			}
			throw new HookError(`onLLMOutput threw: ${err?.message ?? err}`, {
				cause: err
			})
		}
		if (decision.decision === 'continue') {
			messages = Array.isArray(decision.nextHistory)
				? decision.nextHistory.slice()
				: []
			iteration++
			continue
		}
		...
		onEvent({
			type:
				decision.decision === 'finish' ? 'final_pushed' : 'tool_request_pushed',
			sessionId,
			iteration,
			messagesSent
		})
		return {
			status: decision.decision === 'finish' ? 'finished' : 'tool_requested',
			sessionId,
			iteration
		}
	}
...
function assertValidDecision(decision) {
	if (!decision || typeof decision !== 'object') {
		throw new TypeError(
			`onLLMOutput returned invalid decision: ${stringifyForError(decision)}`
		)
	}
	const tag =
	...
	if (typeof tag !== 'string' || !VALID_DECISIONS.has(tag)) {
		throw new TypeError(
			`onLLMOutput returned invalid decision tag: ${stringifyForError(tag)}`
		)
	}
	const hasSingular = Object.prototype.hasOwnProperty.call(
		decision,
		'pushPayload'
	)
	const hasPlural = Object.prototype.hasOwnProperty.call(
		decision,
		'pushPayloads'
	)
	if (hasSingular) {
	...
	if (pushes.length === 0) {
		throw new TypeError(
			'pushPayloads: [] \u2014 use decision: skip-push to skip notification entirely'
		)
	}
	for (let i = 0; i < pushes.length; i++) {
		...
		if (!p || typeof p !== 'object' || Array.isArray(p)) {
			throw new TypeError(
				`pushPayloads[${i}] must be a plain object, got ${stringifyForError(p)}`
			)
		}
		if (Object.prototype.hasOwnProperty.call(p, 'splitPattern')) {
			throw new TypeError(
				`pushPayloads[${i}].splitPattern is removed in next.4; caller is responsible for splitting`
			)
		}
		if (Object.prototype.hasOwnProperty.call(p, 'messageId')) {
			...
			if (typeof id !== 'string' || id === '') {
				throw new TypeError(
					`pushPayloads[${i}].messageId must be a non-empty string when set, got ${stringifyForError(id)}`
				)
			}
		}
	}
...
async function sendPushWithMaybeBlob(pushPayload, payload, ctx, sessionId) {
	...
	try {
		...
		throw new HookError(
			`pushPayload is not JSON-serializable: ${err?.message ?? err}`,
			{ cause: err }
		)
	}
	if (typeof serialized !== 'string') {
		throw new HookError(
			'pushPayload serialized to a non-string (likely `undefined`)'
		)
	}
	const byteLen = PUSH_PAYLOAD_BYTE_ENCODER.encode(serialized).byteLength
	const maxInline =
		ctx.blobStore &&
		Number.isInteger(ctx.blobStore.maxInlineBytes) &&
		ctx.blobStore.maxInlineBytes > 0
	...
	const ttl =
		Number.isInteger(ctx.blobStore.ttlSeconds) && ctx.blobStore.ttlSeconds > 0
	...
	const payloadObj =
		pushPayload && typeof pushPayload === 'object' ? pushPayload : {}
	const envelope = {
		_blob: true,
		key,
		url: blobUrl,
...
function resolveRuntimeMultipartOptions(ctx) {
	...
	return {
		enabled,
		maxChunkBytes: positiveIntegerOrDefault(
			maxChunkBytes,
			DEFAULT_MULTIPART_CHUNK_BYTES
		),
		ttlMs: positiveIntegerOrDefault(config.ttlMs, DEFAULT_MULTIPART_TTL_MS),
		maxChunks: positiveIntegerOrDefault(
			config.maxChunks,
...
function createInstantHandler(options) {
	...
	if (!options.vapid)
		throw new Error('[amsg-instant] options.vapid is required')
	if (options.webpush !== void 0) {
		const warn = globalThis.console && globalThis.console.warn
		if (typeof warn === 'function') {
			warn(
				'[amsg-instant] options.webpush is deprecated and ignored since 0.3.0. Intercept push delivery via options.fetch in tests.'
			)
		}
	}
	const onEvent =
	...
	const tokenSigningKey = options.tokenSigningKey
		? String(options.tokenSigningKey)
		: ''
	...
	const onLLMOutput2 =
		typeof options.onLLMOutput === 'function' ? options.onLLMOutput : null
	const blobStore = options.blobStore || null
	const maxLoopIterations =
		Number.isInteger(options.maxLoopIterations) && options.maxLoopIterations > 0
	...
	return async function handler(request, envOrRuntime, runtime) {
		...
		if (tokenSigningKey) {
			const tokenError = await verifyBearerToken(
				request,
				tokenSigningKey,
				respond
			)
		...
		if (expectedClientTokenBytes) {
			const tokenError = verifyClientToken(
				request,
				expectedClientTokenBytes,
				respond
			)
			if (tokenError) return tokenError
		}
		...
		if (isContinue && !onLLMOutput2) {
			return respond(400, {
				...
				error: {
					...
					message:
						'/continue \u4EC5\u5728 createInstantHandler \u914D\u7F6E\u4E86 onLLMOutput \u65F6\u53EF\u7528'
				}
			})
		}
		let validation
		...
		try {
			...
			registerWaitUntil(
				work,
				resolveWaitUntil(envOrRuntime, runtime, options),
				onEvent
			)
...
function resolveMultipartOptions(options) {
	...
	if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) {
		throw new TypeError(
			'[amsg-instant] multipart must be a plain object when set'
		)
	}
	const multipart =
		/** @type {Record<string, unknown>} */
	...
	return {
		enabled,
		maxChunkBytes: resolvePositiveInt(
			maxChunkBytes,
			DEFAULT_MULTIPART_CHUNK_BYTES,
			'multipart.maxChunkBytes'
		),
		...
		maxChunks: resolvePositiveInt(
			multipart.maxChunks,
			DEFAULT_MULTIPART_MAX_CHUNKS,
			'multipart.maxChunks'
		),
		maxTotalBytes: resolvePositiveInt(
...
function resolvePositiveInt(value, fallback, fieldName) {
	...
	if (!Number.isInteger(value) || value <= 0) {
		throw new TypeError(
			`[amsg-instant] ${fieldName} must be a positive integer. Got: ${value}`
		)
	}
	return value
}
...
async function handleBlobRead(request, pathname, blobStore, baseHeaders) {
	if (!blobStore || !blobStore.adapter) {
		return new Response(
			JSON.stringify({ error: 'blob_store_not_configured' }),
			{
				status: 404,
				headers: {
					...baseHeaders,
	...
	if (typeof body !== 'string') {
		return new Response(
			JSON.stringify({ error: 'blob_not_found_or_expired' }),
			{
				status: 404,
				headers: {
					...baseHeaders,
...
function buildCorsHeaders(cors) {
	const allowOrigin =
		cors && typeof cors.allowOrigin === 'string' && cors.allowOrigin.trim()
...
async function verifyBearerToken(request, signingKey, respond) {
	...
	const expectedSigBytes = await hmacSha256(
		utf8(signingKey),
		utf8(signingInput)
	)
	let receivedBytes
...
// worker/instant-push/node_modules/@rei-standard/amsg-instant/dist/blob/d1.mjs
function createD1BlobStore(db, opts = {}) {
	...
	return {
		async put(key, body, ttlSeconds) {
			await db
				.prepare(putSql)
				.bind(key, body, Date.now() + ttlSeconds * 1e3)
				.run()
		},
		async read(key) {
...
// worker/instant-push/node_modules/@rei-standard/amsg-instant/dist/index.mjs
...
function buildMultipartPushPayloads2(payload, options = {}) {
	...
	const ttlMs = resolvePositiveInteger2(
		options.ttlMs,
		DEFAULT_MULTIPART_TTL_MS2,
		'ttlMs'
	)
	const id =
		typeof options.id === 'string' && options.id.trim()
...
function resolveRuntimeMultipartOptions2(ctx) {
	...
	return {
		enabled,
		maxChunkBytes: positiveIntegerOrDefault2(
			maxChunkBytes,
			DEFAULT_MULTIPART_CHUNK_BYTES2
		),
		ttlMs: positiveIntegerOrDefault2(config.ttlMs, DEFAULT_MULTIPART_TTL_MS2),
		maxChunks: positiveIntegerOrDefault2(
			config.maxChunks,
			DEFAULT_MULTIPART_MAX_CHUNKS2
		),
		maxTotalBytes: positiveIntegerOrDefault2(
			config.maxTotalBytes,
			DEFAULT_MULTIPART_MAX_TOTAL_BYTES2
		)
	}
}
...
var stripTimestamps = t =>
	t
		.replace(/\[\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}\]\s*/g, '')
		.replace(/^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}\s*/gm, '')
		.replace(/（[上下]午\d{1,2}[：:]\d{2}）/g, '')
		.replace(/\(\d{1,2}:\d{2}\s*[AP]M\)/gi, '')
var stripChineseDate = t =>
	t.replace(/\[\d{4}[-/年]\d{1,2}[-/月]\d{1,2}.*?\]/g, '')
var stripRoleNamePrefix = t => t.replace(/^[\w一-龥]+:\s*/, '')
var stripBusinessTagsForBubble = t =>
	t
		.replace(
			/\[\[(?:ACTION|RECALL|SEARCH|DIARY|READ_DIARY|FS_DIARY|FS_READ_DIARY|DIARY_START|DIARY_END|FS_DIARY_START|FS_DIARY_END|MUSIC_ACTION)[:\s][\s\S]*?\]\]/g,
			''
		)
		.replace(/\[schedule_message[^\]]*\]/g, '')
var stripBusinessTagsForNotification = t =>
...
var stripMarkdownDividers = t =>
	t.replace(/^\s*---\s*$/gm, '').replace(/^\s*[-*+]\s*$/gm, '')
var stripBackticks = t =>
	t
		.replace(/`(\[\[[\s\S]*?\]\])`/g, '$1')
		.replace(/``+/g, '')
		.replace(/(^|\s)`(\s|$)/gm, '$1$2')
...
var replaceMarkdownLinks = t =>
	t.replace(/\[([^\]]+)\]\([^)]+\)/g, '[\u94FE\u63A5\uFF1A$1]')
var replaceSendEmoji = t =>
	t.replace(/\[\[SEND_EMOJI:\s*(.+?)\]\]/g, '[\u8868\u60C5\uFF1A$1]')
var replaceEmojiReverseTag = t =>
	t.replace(
		/\[(?:你|User|用户|System|[\w一-龥]+)\s*发送了表情包[:：]\s*(.*?)\]/g,
		'[\u8868\u60C5\uFF1A$1]'
	)
var replaceHtmlBlocks = t =>
...
function chunkText(text) {
	...
	for (const chunk of lineChunks) {
		const guarded = chunk.replace(/\[{1,2}[^\[\]]*\]{1,2}/g, m =>
			m.replace(/\s/g, SENTINEL)
		)
		const sub = guarded
			.split(cjkSpaceRe)
...
function classifyLLMOutput(text) {
	...
	for (const spec of SIDE_EFFECT_TAGS)
		cleanedText = cleanedText.replace(spec.re, '')
...
function resolveOversizeTransport(env) {
	...
	if (raw) {
		console.warn(
			`[instant-push] Unknown AMSG_OVERSIZE_TRANSPORT="${env.AMSG_OVERSIZE_TRANSPORT}", using multipart.`
		)
	}
	return 'multipart'
}
...
function resolveRequestOversizeTransport(body) {
	const raw = String(
		body?.oversizeTransport || body?.amsgOversizeTransport || ''
	)
		.trim()
		.toLowerCase()
	if (raw === 'multipart') return 'multipart'
...
function withRequestOversizeTransport(env, body) {
	...
	return {
		...
		AMSG_ENABLE_D1_BLOBSTORE:
			requested === 'd1'
				? 'true'
				: requested === 'multipart'
					? 'false'
					: env.AMSG_ENABLE_D1_BLOBSTORE
	}
}
...
async function ensureD1BlobSchema(env) {
	...
	if (!env.DB) {
		console.warn(
			'[instant-push] D1 BlobStore requested but DB binding is missing; falling back to multipart.'
		)
		return false
	}
	if (!d1SchemaReadyPromise) {
		d1SchemaReadyPromise = (async () => {
			...
			console.error(
				'[instant-push] D1 BlobStore schema init failed; falling back to multipart.',
				e
			)
			return false
		})
	...
	if (!received) {
		return utilityJson(401, {
			...
			error: {
				...
				message: 'X-Client-Token required'
			}
		})
	}
...
export { buildPushDecision, src_default as default }
