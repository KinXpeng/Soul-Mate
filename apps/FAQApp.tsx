      ...
      {!activeChangelog && (
        <div className="shrink-0 bg-white/60 backdrop-blur-md border-b border-slate-200/60 px-4 py-2">
          <div className="inline-flex bg-slate-100 rounded-full p-1 gap-1">
            <button
              onClick={() => setTab('faq')}
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${tab === 'faq'
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${tab === 'changelog'
                  ? 'bg-white text-indigo-600 shadow-sm'
                  : 'text-slate-500 active:scale-95'
                }`}
            >
